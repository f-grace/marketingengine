...
def composite_scores(
    save_rates: Sequence[float],
    engage_rates: Sequence[float],
    ...
    if len(lengths) != 1:
        raise ValueError(
            ...
            f"{len(save_rates)}, {len(engage_rates)}, {len(reach_indices)}, "
            f"{len(recency_weights)}"
        )

    z_save = zscores(save_rates)
    z_engage = zscores(engage_rates)
    z_reach = zscores(reach_indices)
